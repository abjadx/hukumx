...
function cleanArticleTextForDisplay(value: string): string {
  ...
  return value
    ...
    .replace(/[）]/g, ')')
    .replace(/[ \t]+/g, ' ')
    // حذف الأقواس الفارغة بعد الأرقام: 170() أو 170( ) => 170
    .replace(/(\d+)\s*\(\s*\)/g, '$1')

    // حذف القوس المفتوح بعد الرقم إذا جاء بهذا الشكل: 2( أو 3 (
    .replace(/^\s*(\d+)\s*\(\s*/gm, '$1. ')

    // حذف القوس المغلق بعد الرقم إذا جاء بهذا الشكل: 2) أو 3 )
    .replace(/^\s*(\d+)\s*\)\s*/gm, '$1. ')
