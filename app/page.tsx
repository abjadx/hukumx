...
export default function Home() {
  ...
  const submitIntake = () => {
    if (!intakeData.verdictType || !intakeData.notificationDate || !intakeData.court || !intakeData.role) return;
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800">
      ...
      <div className="flex flex-col items-center px-4 py-12">
        ...
        <div className="w-full max-w-2xl space-y-4">
          ...
          {/* Step 1 - Country */}
          <div
            className={`bg-slate-700 rounded-2xl p-4 border-2 transition-all ${!country && formError.includes('الدولة') ? 'border-red-400' : 'border-transparent'}`}
            dir="rtl"
          >
          ...
          {/* Step 2 - Case Type */}
          <div
            className={`bg-slate-700 rounded-2xl p-4 border-2 transition-all ${!caseType && formError.includes('القضية') ? 'border-red-400' : 'border-transparent'}`}
            dir="rtl"
          >
            <p className="text-slate-300 text-sm mb-3 font-medium">
        ...
        {/* Case Intake Form */}
        {showIntake && !loading && (
          <div className="w-full max-w-2xl mt-6 bg-slate-700 rounded-2xl p-6 border border-amber-500/50 shadow-xl" dir="rtl">
            <div className="flex items-center gap-2 mb-5 pb-3 border-b border-slate-600">
              <span className="text-amber-400 text-xl">📋</span>
              <h3 className="text-amber-400 font-bold text-lg">تفاصيل الاستئناف / الحكم</h3>
            </div>
            <p className="text-slate-300 text-sm mb-5">
              لتقديم استشارة دقيقة، نحتاج بعض التفاصيل حول قضيتك:
            </p>
            ...
            <div className="space-y-5">
              ...
              <div>
                <label className="text-slate-300 text-sm font-medium block mb-2">
                  نوع الحكم <span className="text-red-400">*</span>
                </label>
                <div className="flex flex-wrap gap-2">
                  {['حكم ابتدائي', 'حكم استئناف', 'حكم تمييز', 'أمر قضائي', 'حكم تحكيم'].map(type => (
                    <button
                      key={type}
                      onClick={() => setIntakeData(p => ({ ...p, verdictType: type }))}
              ...
              <div>
                <label className="text-slate-300 text-sm font-medium block mb-2">
                  تاريخ التبليغ بالحكم <span className="text-red-400">*</span>
                </label>
                <input
                  type="date"
                  value={intakeData.notificationDate}
                  onChange={e => setIntakeData(p => ({ ...p, notificationDate: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setIntakeData(p => ({ ...p, court: e.target.value }))}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {['مدعي', 'مدعى عليه', 'محكوم عليه', 'محكوم له', 'طرف ثالث'].map(role => (
                    <button
                      key={role}
                      onClick={() => setIntakeData(p => ({ ...p, role }))}
                      className={`px-4 py-2 rounded-xl text-sm border transition-all ${
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={e => setIntakeData(p => ({ ...p, details: e.target.value }))}
                  placeholder="أي معلومات إضافية تساعد في تقديم استشارة أدق..."
                  rows={3}
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  disabled={!intakeData.verdictType || !intakeData.notificationDate || !intakeData.court || !intakeData.role}
